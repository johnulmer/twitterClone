...
public class TwitterRouter {
	...
	private static void checkSession(Request request) {
		...
		// TODO change from void to boolean, check session(user.userID), return
		// false if not greater than -1
	...
	private static String authenticate(Request request, Response response) {
		User u = new User(request.queryParams("username"), request.queryParams("password"), "", -1);
		u = u.authenticate();
		if (u.getUserID() > -1) {
	...
	// attempt to process registration for new site user or return message re:
	// username or handle already in use
	...
	// retrieve a User from session & new password and handle from request,
	// process if valid
	...
	/*
	 * TO STOP FOLLOWING: // present page of users currently followed //
	 * userStopFollowing(request, response);
	 * 
	 * // return the specific list of users who ARE followed by the logged in
	 * User // showFollowedUsers(request, response);
	 * 
	 * // handle request to STOP following a specific user //
	 * stopFollowingThisUser(request, response);
	 */

	// retrieve a User from session, show all users who ARE followed by that
	// User
	...
	// retrieve a User from session, show other users who ARE followed by that
	// user
	...
	// retrieve a User from session & userID from request, STOP following that
	// User
	...
	public static void main(String[] args) {
		...
		// show log in, auth, register. login page is a template because it
		// needs to handle header include
